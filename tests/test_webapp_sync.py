...
естественный для vitest способ добраться до styles.css — ИМПОРТ — вечнозелёный:
при `css: false` (webapp/vite.config.ts) содержимое доезжает до теста пустой
строкой, и обходной `?raw` тоже (пробник: 0 символов у обоих). Читать файл с
диска vitest умеет — тот же файл через `node:fs` отдаёт все 19 996 символов
вместе с `--ink`, — но такая проверка делала бы ровно то, что уже делает этот
питоновский прогон, и стояла бы отдельно от остальных сверок копий. Поэтому
обе живут здесь, рядом с ними.
"""
import ast
import json
import pathlib
