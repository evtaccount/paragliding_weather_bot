...
// route обрывает запрос внутри браузера, до обращения к DNS.
//
// Хвост `*` в шаблоне обязателен: документация Telegram предлагает закреплять
// версию Bot API строкой запроса (`…/telegram-web-app.js?57`), и шаблон без
// него перестал бы совпадать вовсе. Тогда настоящий скрипт загружается,
// затирает подставленный window.Telegram, приложение показывает «Не Telegram»
// — и падают 7 сценариев из 8, причём ни одно сообщение об отказе на адрес
// скрипта не указывает. Проверено дописыванием `?57` в index.html.
