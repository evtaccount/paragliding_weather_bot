...
export function Forecast({ site, date, model, active = true }: ForecastProps) {
  ...
  // build_url её для них не читает вовсе, и «Обзор» передаёт null намеренно
  // (см. шапку Overview.tsx). Без даты обязателен только прогноз на день.
  const forecast = useForecast(site, "1d", date, model, active && date !== null)

  // Пока пилот не назвал старт и день, экран говорит, чего не хватает, а
  // запрос не уходит вовсе (просьба владельца, бриф explicit-site-and-day:
  // приложение открывалось готовым прогнозом, о котором никто не просил).
