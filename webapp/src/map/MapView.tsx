...
// всё равно нужно на что-то навести. Наводится она на первый старт
// ПОСТАВОЧНОЙ библиотеки — sites.json в корне репозитория, тот самый файл,
// который приезжает на сервер и раздаётся ботом. Раньше здесь стояли
// координаты Гудаури из тестовой фикстуры (tests/fixtures.py,
// test/fixtures/sites.json) — старта, которого в поставке нет вовсе
// (финальное ревью ветки, m5). Совпадение с sites.json держит
// tests/test_webapp_sync.py.
const FALLBACK_CENTER: L.LatLngExpression = [42.089329, 45.401151]
...
export function MapView({ points, sites, onTap, onDragPoint, onPointTap }: Props) {
  ...
  useEffect(() => {
    ...
    // (читается через ref). Директивы отключения линтера тут нет: eslint в
    // проекте не установлен и не настроен (package.json), и такая строка
    // молча обещала бы читателю проверку, которой не существует. Отсутствие
    // директив при отсутствии линтера держит tests/test_webapp_sync.py.
  }, [])
