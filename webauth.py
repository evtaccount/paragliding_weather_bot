...
def data_check_string(pairs: list[tuple[str, str]]) -> str:
    ...
    Это не только чтение документации: страница-проба открывалась в НАСТОЯЩЕМ
    клиенте Telegram, и подпись сходилась — то есть `signature` внутри строки
    и есть то, что Telegram подписывает. Записано здесь потому, что проверить
    это тестами нельзя (в них initData подписываем мы сами, tests/tma.py), а
    ошибка в эту сторону не видна ничем, кроме 401 у каждого живого пилота на
    раскатке.
    """
