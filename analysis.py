...
Не пересказывай все числа подряд — приводи только те, что влияют на решение. Без markdown-таблиц. В конце — одна строка-оговорка: перескажи то, что пришло в caveats, и добавь, что осталось непроверенным (unchecked_vetoes), если такое есть. Своих оговорок не придумывай: совет переснять прогноз лежит в caveats и появляется там, только когда до дня ещё есть время — на сегодняшнем и завтрашнем дне его нет, и писать его нельзя."""
