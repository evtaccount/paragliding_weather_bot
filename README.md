...
**Метеомодель.** `/model` переключает источник (open-meteo `&models=`): `auto`
(`best_match`), `ecmwf`, `gfs`, `icon`. По умолчанию — **ECMWF**. ECMWF не отдаёт
высоту пограничного слоя и уровень замерзания, поэтому на ECMWF строка «🧗 Потолок»
показывает «н/д» и график потолка термиков не строится (остальное — метеограмма,
профиль ветра, сетка высот — работает); `auto`/`icon` потолок дают. Текущая модель
подписана в карточке прогноза.
