...
| `/model [auto\|ecmwf\|gfs\|icon]` | выбор метеомодели (деф. ECMWF); без аргумента — кнопки-пикер; сменить можно и кнопкой под прогнозом |
...
в ячейке стрелка (откуда) и скорость, цвет — сила ветра.

**Метеомодель.** Источник open-meteo (`&models=`): `auto` (`best_match`), `ecmwf`,
`gfs`, `icon`. По умолчанию — **ECMWF**. Сменить: командой `/model` (без аргумента —
...
Нюанс потолка: `boundary_layer_height` (потолок термиков) на open-meteo отдают
только `auto` (best_match) и `gfs`; у `ecmwf` и `icon` его нет — на них строка
«🧗 Потолок» показывает «н/д» и график потолка не строится (метеограмма, профиль
ветра, сетка высот — работают на любой модели).
